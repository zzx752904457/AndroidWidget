...
//                    mLastMotionX = event.getX();
                    ...
                    if(pointIndex == -1) {
//                        Log.i(TAG, "move----pointer id          :"+pointId);
//                        Log.i(TAG, "move----point index         :"+event.findPointerIndex(pointId));
                        return false;
                    }
                    mLastMotionY = event.getY(pointIndex);
                    mLastMotionX = event.getX(pointIndex);

                    distance = mLastMotionY - mInitialMotionY;
...
//                    Log.i(TAG, "mInitialMotionY:"+mInitialMotionY);
            ...
            case MotionEvent.ACTION_POINTER_DOWN:
                ...
                /*Log.i(TAG, "point down----action index:"+actionIndex);
                Log.i(TAG, "point down----id          :"+id);
                Log.i(TAG, "point down----point  index:"+pointerIndex);*/

                pointX = event.getX(pointerIndex);
                pointY = event.getY(pointerIndex);
//                Log.i(TAG, "point down----id:"+pointId+"\n point y:"+pointY);
                return true;
            case MotionEvent.ACTION_POINTER_UP:

                /*Log.i(TAG, "point up----action index:"+actionIndex);
                Log.i(TAG, "point up----id          :"+id);
                Log.i(TAG, "point up----point  index:"+pointerIndex);
                Log.i(TAG, "point up----point  leave:"+actionIndex);*/

                // 获取离开屏幕的手指的索引
                int pointerIdLeave = event.getPointerId(actionIndex);

                if (pointerIdLeave == pointId) {
                    pointId = event.getPointerId(pointerIndex == 0 ? 1 : 0);

//                    pointId = pointerIdLeave == 0 ? 1 : 0;
                    mInitialMotionY = pointY - distance;
                    mLastMotionY = pointY;
                    Log.i(TAG, "p up af------id change:"+pointId);
                }
                return true;
    ...
    protected final void refreshLoadingViewsSize() {
        final int maximumPullScroll = (int) (getMaximumPullScroll() * 1.0f);
        ...
        if (DEBUG) {
            Log.d(TAG, String.format("Setting Padding. L: %d, T: %d, R: %d, B: %d", pLeft, pTop, pRight, pBottom));
        }
    ...
    private void pullEvent() {
        ...
        switch (mCurrentMode) {
            ...
            case PULL_FROM_START:
            default:
                newScrollValue = Math.round(Math.min(initialMotionValue - lastMotionValue, 0) / 3);
    ...
    private int getMaximumPullScroll() {
        switch (getPullToRefreshScrollDirection()) {
            ...
            default:
                ...
                BitmapFactory.decodeResource(getResources(),R.drawable.bg_refreshing,options);

//                return Math.round(options.outHeight);
                return Math.round(getHeight() * 1.2f);
        }
    }
